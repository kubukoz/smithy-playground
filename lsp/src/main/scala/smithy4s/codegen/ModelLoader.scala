...
object ModelLoader {
  ...
  def load(
    ...
    val serviceFactory = ProjectionTransformer.createServiceFactory(validatorClassLoader)
    ...
    val transformedModel =
      trans.foldLeft(model)((m, t) => t.transform(TransformContext.builder().model(m).build()))
